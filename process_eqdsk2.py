...
def add_extra_data(val):
    ...
    f1 = RectBivariateSpline(rgrid, zgrid, val["btrz"])
    f2 = RectBivariateSpline(rgrid, zgrid, val["bzrz"])
    ...
    val['btmid'] = np.array([f1(r, val["zmaxis"])
                             for r in rgrid]).flatten()
    f = RectBivariateSpline(rgrid, zgrid, val["jtrz"])
    val['jtmid'] = np.array([f(r, val["zmaxis"])
                             for r in rgrid]).flatten()
    f = RectBivariateSpline(rgrid, zgrid, val["pressrz"])
    val['pressmid'] = np.array([f(r, val["zmaxis"])
                                for r in rgrid]).flatten()
    f = RectBivariateSpline(rgrid, zgrid, val["qrz"])
    val['qmid'] = np.array([f(r, val["zmaxis"])
                            for r in rgrid]).flatten()


def getGfileDict(eqdsk_file):
